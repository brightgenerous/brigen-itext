...
import com.brightgenerous.itext.writer.creater.DocumentCreater;
import com.brightgenerous.itext.writer.creater.DocumentCreaters;
...
public class PdfWriterBuilder<T> {
    ...
    private static final Boolean DEFAULT_SMART_MERGE = Boolean.FALSE;

    private IPagesAppender pagesAppender;

    private CreaterHolder<T> emptyCreaterHolder;

    private Boolean smartMerge;
    ...
    public PdfWriterBuilder<T> emptyCreater(IResourceLoader<T> resource,
        ...
        emptyCreaterHolder = holder;
        return this;
    }

    // smartMerge

    public Boolean smartMerge() {
        return smartMerge;
    }

    public PdfWriterBuilder<T> smartMerge(Boolean smartMerge) {
        this.smartMerge = smartMerge;
        return this;
    }

    public PdfWriterBuilder<T> smartMerge(boolean smartMerge) {
        return smartMerge(smartMerge ? Boolean.TRUE : Boolean.FALSE);
    }
    ...
    protected IPdfWriterStrategy<T> getStrategy() {
        return new PdfWriterStrategy<>(getCreaters(), getEmptyCreater(), getSmartMerge());
    }
    ...
    protected IDocumentCreater<T> getEmptyCreater() {
        CreaterHolder<T> holder = emptyCreaterHolder;
        if (holder == null) {
    ...
    protected boolean getSmartMerge() {
        Boolean ret = smartMerge;
        if (ret == null) {
            ret = DEFAULT_SMART_MERGE;
        }
